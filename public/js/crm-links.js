...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const interactionDescriptions = document.querySelectorAll('.interaction-description, .activity-description, .notes-section');
    ...
    interactionDescriptions.forEach(function(element) {
        ...
        html = html.replace(/\{Lead #(\d+)\}/g, function(match, leadId) {
            return `<a href="/leads/${leadId}" class="entity-link lead-link" title="View Lead #${leadId}">{Lead #${leadId}}</a>`;
        });

        // Convert {Customer #123} to clickable link
        html = html.replace(/\{Customer #(\d+)\}/g, function(match, customerId) {
            return `<a href="/customers/${customerId}" class="entity-link customer-link" title="View Customer #${customerId}">{Customer #${customerId}}</a>`;
        });
